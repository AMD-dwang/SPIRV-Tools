...
class CooperativeMatrixKHR : public Type {
 public:
  CooperativeMatrixKHR(const Type* type, const uint32_t scope, const uint32_t rows,
      const uint32_t columns, const uint32_t use, const uint32_t component_Interpretation);
  ...
  uint32_t component_type_Interpretation() const { return component_type_Interpretation_; }
 ...
 private:
  ...
  const uint32_t component_type_Interpretation_;
};
